...
test('刷新、logout 和 401 使用正式认证生命周期', async () => {
  ...
  assert.match(store, /sessionStorage\.getItem\(STORAGE_KEY\)/)
  assert.match(store, /sessionStorage\.setItem\(STORAGE_KEY/)
  assert.doesNotMatch(store, /localStorage\.(getItem|setItem)\(STORAGE_KEY/)
