<template>
  <div class="action-bar">
    <el-button v-if="application?.status === 'DRAFT' && isMigrationDraft" type="primary" @click="$emit('continue-migration')">继续编辑并提交</el-button>
    <el-button v-else-if="application?.status === 'DRAFT' && canSubmit" type="primary" :loading="loading" @click="$emit('submit')">提交申请</el-button>
...
const props = defineProps({ application: { type: Object, default: null }, loading: Boolean, isMigrationDraft: Boolean })
defineEmits(['submit', 'withdraw', 'cancel', 'continue-migration'])
