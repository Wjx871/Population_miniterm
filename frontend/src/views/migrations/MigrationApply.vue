  ...
  <div class="page-container">
    ...
    <el-card shadow="never" class="form-card">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px" @submit.prevent>
        ...
        <el-form-item label="办理人员" prop="personId"><PersonSelect v-model="form.personId" :disabled="isReadonly" /></el-form-item>
        ...
        <template v-else>
          ...
          <el-form-item v-if="requiresNewHead" label="指定新户主"><MemberCandidateSelect v-model="form.newHeadPersonId" :candidate-ids="candidateIds" :disabled="isReadonly" /><div class="form-tip">若不指定新户主，请上传家庭成员同意材料。</div></el-form-item>
        </template>
        ...
        <el-form-item v-if="isIn" label="迁移批次号"><el-input v-model.trim="form.transferBatchNo" :disabled="isReadonly" maxlength="40" /></el-form-item>
        <el-form-item><el-button v-if="!isReadonly" type="primary" :loading="saving" @click="saveDraft">保存草稿</el-button><el-button @click="router.back()">返回</el-button></el-form-item>
      </el-form>
    </el-card>
    <template v-if="applicationId">
      <MaterialUploader v-if="!isReadonly && canUpload" :application-id="applicationId" @uploaded="refreshDetail" />
      ...
      <ApplicationActionBar :application="detail?.application" :loading="submitting" @submit="submit" @withdraw="withdraw" />
    </template>
  </div>
</template>
...
import { submitApplication, withdrawApplication } from '../../api/applications'
...
const applicationId = ref(route.query.applicationId || null); const detail = ref(null); const saving = ref(false); const submitting = ref(false); const formRef = ref()
...
const rules = computed(() => ({ personId: [{ required: true, message: '请选择办理人员', trigger: 'change' }], migrationType: [{ required: true, message: '请选择迁移类型', trigger: 'change' }], title: [{ required: true, message: '请输入申请标题', trigger: 'blur' }], reason: [{ required: true, message: '请输入迁移原因', trigger: 'blur' }], toRegionCode: [{ required: true, message: '请输入行政区划代码', trigger: 'blur' }], ...(isIn.value ? { fromAddress: [{ required: true, message: '请输入迁出地地址', trigger: 'blur' }], toHouseholdId: [{ required: true, message: '请选择目标家庭户', trigger: 'change' }], inDate: [{ required: true, message: '请选择迁入日期', trigger: 'change' }] } : { toAddress: [{ required: true, message: '请输入迁往地地址', trigger: 'blur' }], outDate: [{ required: true, message: '请选择迁出日期', trigger: 'change' }] }) }))
...
<style scoped>.page-container{display:flex;flex-direction:column;gap:16px}.page-header h1{margin:0 0 8px}.subtitle,.form-tip{color:var(--el-text-color-secondary);font-size:13px}.form-card{max-width:860px}</style>
