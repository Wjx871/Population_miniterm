...
/* 横幅入口保持同一基线，避免遮挡中间插画；层叠交互由下方“常用入口”承担。 */
.hero-actions{width:372px;transform:translate(-2px,-8px)}
.action-card-stack{display:flex;width:372px;height:118px;gap:10px;align-items:stretch}
.action-card{position:relative;left:auto;top:auto;width:calc((100% - 20px)/3);height:112px;min-width:0;padding:12px 11px;border-radius:13px;transform:none;filter:none}
.card-1,.card-2,.card-3{z-index:auto;transform:none;filter:none}
.action-card:hover,.card-1:hover,.card-2:hover,.card-3:hover{z-index:2;filter:none;transform:translateY(-8px)}
.action-card strong{margin-top:8px;font-size:14px;white-space:nowrap}.card-description{margin-top:4px;font-size:11px;white-space:nowrap}.card-arrow{right:11px;bottom:9px}

@media(max-width:1280px){.hero-banner{grid-template-columns:minmax(300px,1fr) minmax(250px,.75fr) auto;padding-inline:28px}.ill-img{width:400px}.hero-actions,.action-card-stack{width:326px}.action-card{width:calc((100% - 16px)/3);padding-inline:9px}.stat-grid{grid-template-columns:repeat(2,minmax(0,1fr));grid-auto-rows:115px}}
@media(max-width:900px){.hero-banner{display:flex;flex-direction:column;align-items:flex-start;gap:22px;height:auto;padding:26px}.hero-actions{width:100%;transform:none}.hero-illustration{display:none}.action-card-stack{width:100%;max-width:372px}.stat-grid,.content-grid{grid-template-columns:1fr}.stat-grid{grid-auto-rows:115px}}</style>
