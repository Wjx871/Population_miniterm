...
def application_rows() -> list[tuple[object, ...]]:
    ...
    def add(kind: str, number: int, target: int, status: str, approval: str, offset: int) -> None:
        rows.append(
            (
    ...
    for number in range(1, 351):
        add("PERMIT", number, 720 + number, "COMPLETED", "COMPLETED", number % 30)
    for number in range(1, 91):
        add("MIGRATION_IN", number, number, "COMPLETED", "COMPLETED", number % 30)
    for number in range(1, 81):
        add("MIGRATION_OUT", number, 90 + number, "COMPLETED", "COMPLETED", number % 30)
    for number in range(1, 41):
        add("CANCELLATION", number, 170 + number, "UNDER_REVIEW", "PENDING", number % 30)
    for number in range(1, 26):
        add("DATA_EXPORT", number, 210 + number, "UNDER_REVIEW", "PENDING", number % 30)
    approval_states = ("PENDING", "APPROVED", "REJECTED", "COMPLETED")
    application_states = ("SUBMITTED", "APPROVED", "REJECTED", "COMPLETED")
...
def generate(database: str, output: Path, people: int) -> None:
    ...
    with output.open("w", encoding="utf-8", newline="\n") as handle:
        ...
        handle.write(
            ...
            "SELECT a.application_no,CASE a.business_type WHEN 'PERMIT' THEN 'RESIDENCE_PERMIT_FIRST_ISSUE' ELSE a.business_type END,a.title,\n"
            ...
            "       CASE WHEN a.approval_status='PENDING' THEN @demo_approver_user ELSE NULL END,@demo_population_department,'110105',@demo_population_user,\n"
            ...
            "SELECT CONCAT('DEMO-CANCEL-',LPAD(s.seq-170,4,'0')),ba.application_id,'PERSON',p.person_id,h.household_id,'OTHER','批量生成的待办注销演示',CURRENT_DATE,\n"
