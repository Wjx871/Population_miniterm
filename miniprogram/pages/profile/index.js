...
Page({
  ...
  onShow() { this.setData({ user: normalizeUser(getApp().globalData.user) }); this.loadHealth() },
  ...
  async logout() {
    ...
    finally { storage.clearSession(); getApp().setUser(null); this.setData({ loggingOut: false }); wx.reLaunch({ url: '/pages/login/index' }) }
  }
})
