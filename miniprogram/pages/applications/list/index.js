...
Page({
  ...
  onShow() { if (!this._needsRefresh) return; this._needsRefresh = false; return this.load(true) },
  inputNo(e) { this.setData({ applicationNo: e.detail.value }) }, changeStatus(e) { this.setData({ statusIndex: Number(e.detail.value) }) },
  search() { return this.load(true) }, reset() { this.setData({ applicationNo: '', statusIndex: 0 }); return this.load(true) },
  async load(reset) { if (this.data.loading || this.data.loadingMore || (!reset && (this.data.last || this.data.error))) return; const page = reset ? 0 : this.data.page + 1; this.setData(reset ? { loading: true, error: '' } : { loadingMore: true }); try { const status = this.data.statusOptions[this.data.statusIndex].value; const result = adapter.normalizePage(await service.list({ applicationNo: this.data.applicationNo.trim() || undefined, status: status || undefined, page, size: this.data.size })); this.setData({ records: reset ? result.records : this.data.records.concat(result.records), page: result.number, total: result.total, last: result.last || (page + 1) * this.data.size >= result.total, error: '' }) } catch (error) { if (reset) this.setData({ records: [], error: messageOf(error) }); else wx.showToast({ title: messageOf(error), icon: 'none' }) } finally { this.setData({ loading: false, loadingMore: false }); wx.stopPullDownRefresh() } },
  onPullDownRefresh() { return this.load(true) }, onReachBottom() { return this.load(false) }, open(e) { wx.navigateTo({ url: `/pages/applications/detail/index?id=${e.currentTarget.dataset.id}` }) }
})
