...
test('403 提示当前账号无权使用且不清理 Token', () => { storageApi.setToken('valid'); assert.throws(() => requestApi.parseResponse({ statusCode: 403, data: { code: 403 } }), /没有此功能的使用权限/); assert.equal(storageApi.getToken(), 'valid'); assert.equal(calls.toast.at(-1), '当前账号无权使用') })
test('400 展示后端具体消息', () => assert.throws(() => requestApi.parseResponse({ statusCode: 400, data: { code: 400, message: '用户名不能为空' } }), /用户名不能为空/))
test('404 使用记录可能已更新文案', () => assert.throws(() => requestApi.parseResponse({ statusCode: 404, data: { code: 404 } }), /未找到对应记录，记录可能已被更新/))
test('409 展示业务冲突消息', () => assert.throws(() => requestApi.parseResponse({ statusCode: 409, data: { code: 409, message: '审批已被处理' } }), /审批已被处理/))
test('500 不伪装成功或空数据', () => assert.throws(() => requestApi.parseResponse({ statusCode: 500, data: { code: 500 } }), /暂时无法完成操作/))
