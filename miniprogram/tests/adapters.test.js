...
test('材料状态和必交标记中文化', () => { const row = application.material({ verifyStatus: 'VERIFIED', requiredFlag: true, materialId: 2 }); assert.equal(row.verifyStatusDisplay, '已核验'); assert.equal(row.requiredDisplay, '必交材料'); assert.equal(row.viewable, true) })
