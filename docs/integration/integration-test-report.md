...
- 执行前数据核对：700001 通用申请、流动人口和最终居住证存在，但专业申请缺失；执行幂等 DEMO SQL 后专业记录已补齐。700002 通用申请和注销专业记录存在。王家兴没有当前有效户籍，不补造用户测试数据。
- DEMO SQL：`scripts/data/fix-manual-acceptance-demo.sql` 仅匹配 `DEMO-PERMIT-001`，使用 `NOT EXISTS` 保证幂等；用户已在正式库执行，重复执行未产生重复数据。
...
- 正式库接口复核：`GET /api/residence-permits/applications/700001` 返回 HTTP 200、API code 200；申请号 `DEMO-PERMIT-001`，专业状态 `COMPLETED`，关联已签发证件 700001。
- 最终结论：FUNC-PERMIT-001 从 DATA_REQUIRED 更新为 CLOSED，人工验收集中修复的 9 项问题全部 CLOSED。
