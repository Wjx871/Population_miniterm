...
正式资源以本地 SVG data URI 形式集中生成，通过 CSS mask 继承页面颜色。没有字体、CDN、远程 URL、演示页或全量图标包。当前仅包含登录页、工作台、导航及人口页面实际需要的 25 个底层资源。底部导航使用 `dashboard`、`database`、`application`、`profile` 四个语义。
