...
正式资源以本地 SVG data URI 形式集中生成，通过 CSS mask 继承页面颜色。没有字体、CDN、远程 URL、演示页或全量图标包。当前仅包含登录页、工作台、导航及人口和家庭户页面实际需要的 26 个底层资源。底部导航使用 `dashboard`、`database`、`application`、`profile` 四个语义。
...
用户提供的微信开发者工具截图已确认首批 SVG data URI 和 CSS mask 图标可以正常显示，不是方框或乱码。第六阶段已找到本机 CLI，但开发者工具服务端口未开启，因此未能自动复核全部图标、Console、离线和打包路径。
...
- 未映射到本地资源的语义名称会安全地渲染为空；新页面引用前必须先完成来源登记、资源映射和目标基础库验证。
