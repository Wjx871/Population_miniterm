...
正式资源以本地 SVG data URI 形式集中生成，通过 CSS mask 继承页面颜色。没有字体、CDN、远程 URL、演示页或全量图标包。当前仅包含登录页、工作台和首个列表箭头实际需要的 22 个底层资源。
...
用户提供的微信开发者工具截图已确认首批 SVG data URI 和 CSS mask 图标可以正常显示，不是方框或乱码。本机仍未发现开发者工具 CLI，新增图标的控制台、离线和打包路径需要继续人工复核。
