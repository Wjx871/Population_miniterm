...
REM ---------- 加载本地 env 文件（与 start_backend.bat 同一套配置） ----------
...
if not exist "%ROOT_DIR%\start_backend.bat" (
    echo [错误] 未找到 start_backend.bat
    goto :fail
)
if not exist "%ROOT_DIR%\start_frontend.bat" (
    echo [错误] 未找到 start_frontend.bat
    goto :fail
)
...
REM ---------- 2. 启动后端（新窗口） ----------
echo [1/3] 正在新窗口启动后端...
start "PDMS-Backend" /D "%ROOT_DIR%" cmd /k call "%ROOT_DIR%\start_backend.bat"
...
call "%ROOT_DIR%\start_frontend.bat"
