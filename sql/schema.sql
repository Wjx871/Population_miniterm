...
CREATE TABLE sys_approval_request (
    ...
    business_type VARCHAR(50) NULL COMMENT '业务类型（PERSON_CREATE/HOUSEHOLD_ESTABLISH/MIGRATION_IN/MIGRATION_OUT/PERSON_UPDATE）',
    business_id BIGINT NULL COMMENT '业务主键ID（PERSON_UPDATE 等已有业务时填）',
    payload_json JSON NULL COMMENT '业务载荷 JSON（结构化字段，独立于 apply_reason，避免字符串拼接注入）',
    apply_reason VARCHAR(500) NULL COMMENT '申请原因（仅用户提交的自由文本）',
