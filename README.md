...
后端重点人口建档/解除/历史与显式执行已完成（V4_009）。刑满释放恢复登记、多级审批、补换领、真实制卡和政务联网仍未实现。
...
已有数据库在备份后按文件名顺序执行 V4_001 至 V4_012。不得将初始化工具当作旧库升级工具。也可以先在明确命名的测试库中运行：
...
登录、JWT、五角色权限与数据范围、统一 person 人口主模型、严格身份证校验、家庭户与成员管理、安全户主变更、行政区划、数据字典、通用证件、业务申请、材料上传、单级审批、迁入迁出、注销与家庭户归档、流动人口、居住证、重点人口建档/解除/历史与显式执行、普通/敏感导出与审计、综合查询统计、操作/登录日志只读查询。专业业务审批通过后仍需授权人员显式执行。

### 前端最终阶段（`feat/frontend-population`）

在既有人口/户籍/迁移/流动/居住证/字典/区划能力之上，已接入：

| 模块 | 前端能力 |
|------|----------|
| 注销 | 综合列表、人员/家庭户申请、材料审批、显式执行、家庭户归档只读 |
| 导出 | 普通脱敏导出（创建记录后鉴权下载）、敏感导出申请/审批/执行/下载、导出记录 |
| 重点人口 | 列表/详情/历史、建档与解除申请、专业 submit、显式执行（权限码 `key-population:*`） |
| 日志 | 操作日志列表/详情、登录日志列表（`log:view`，无删除） |
| 框架 | 申请/审批详情 Handler 泛化，支持新业务专业面板与执行文案 |

演示账号仍为 `viewer / population / household / approver / admin`，课程环境初始密码 `123456`。

### 明确不做（不属于当前验收范围）

刑满释放恢复登记、多级审批、用户/角色在线 CRUD、消息队列、微服务、云存储、真实制卡、政务平台对接、短信和邮件、物理删除注销/重点人口/日志/导出记录。
